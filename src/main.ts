...
function demoBanner(): string {
  return `<aside class="demo-banner screen-only" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved</strong><span>Four sample artifacts show a six-week portfolio.</span><div><button type="button" data-action="reset-demo">Reset demo</button><button type="button" data-action="start-real">Start for real</button></div></aside>`;
}
...
function legalPage(kind: "privacy" | "terms"): string {
  const privacy = `<p class="eyebrow">Plain-language policy</p><h1 tabindex="-1">How your work stays private</h1><p class="lede">Your family’s work stays under your control.</p>
    <h2>What this site stores</h2><p>The site stores challenge choices, custom challenges, artifact notes, and rubric scores in this browser. It has no user accounts. It does not upload your portfolio.</p>
    <h2>What leaves your device</h2><p>Normal use sends no portfolio data away. This release offers no checkout or cloud sync.</p>
    ...
    <h2>Children</h2><p>The site asks for no child name, email address, photo, or public profile. Adults should avoid sensitive details in artifact notes.</p>
...
function deckSection(): string {
  ...
  return `<section id="deck" class="section deck-section" aria-labelledby="deck-title"><div class="section-heading"><div><p class="eyebrow">Challenge deck</p><h2 id="deck-title" tabindex="-1">Choose a printable challenge</h2></div><p>Filter by skill or age. Each challenge includes materials, limits, reflection, and review.</p></div>
...
function howSection(): string {
  return `<section class="section how-section" aria-labelledby="how-title"><div class="section-heading"><div><p class="eyebrow">How it works</p><h2 id="how-title" tabindex="-1">Turn a challenge into reviewable work</h2></div><p>Use the same short cycle for each activity.</p></div><ol class="work-steps"><li><span>01</span><h3>Choose a challenge</h3><p>Pick an age range and skill mode.</p></li><li><span>02</span><h3>Make and review</h3><p>Keep the work, then use the adult or peer rubric.</p></li><li><span>03</span><h3>Log an artifact</h3><p>Record the evidence, one observation, and a next step.</p></li></ol></section>`;
}

function privacySection(): string {
  return `<section class="section boundaries-section" aria-labelledby="boundaries-title"><div><p class="eyebrow">Private by design</p><h2 id="boundaries-title" tabindex="-1">Your family controls the work</h2></div><ul><li>Portfolio notes stay in this browser unless you export them.</li><li>No child account or public profile.</li><li>No AI scoring or career predictions.</li></ul><a href="/privacy">Read the privacy policy</a></section>`;
}
  ...
  const metadata = route.page === "privacy"
    ...
    : route.page === "terms"
      ...
      : route.page === "not-found"
        ...
        : route.demo
          ? { title: `Demo — ${PRODUCT}`, description: "Explore a sample six-week portfolio without changing your family’s saved work.", path: "/demo" }
...
function bindDialogEvents(): void {
  ...
  document.querySelector<HTMLFormElement>("#artifact-form")?.addEventListener("submit", (event) => {
    ...
    persist(route.demo ? "Artifact added to the demo." : "Artifact saved on this device.");
    closeDialog();
    render();
...
function bindEvents(): void {
  ...
  document.querySelectorAll<HTMLElement>("[data-action]").forEach((element) => element.addEventListener("click", (event) => {
    ...
    if (action === "delete-artifact") { const artifact = state.artifacts.find((item) => item.id === id); if (artifact && confirm(`Delete “${artifact.title}” from this browser?`)) { state.artifacts = state.artifacts.filter((item) => item.id !== id); persist("Artifact deleted."); render(); } }
