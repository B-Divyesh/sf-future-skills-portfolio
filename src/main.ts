...
function legalPage(kind: "privacy" | "terms"): string {
  ...
  const terms = `<p class="eyebrow">Use terms</p><h1>Terms of use</h1><p class="lede">A durable family learning tool, offered without career predictions or automated judgments.</p>
    ...
    <h2>Keepsake Deck purchase</h2><p>The optional Keepsake Deck is a $19 USD one-time purchase that unlocks the extended curated challenge set and keepsake print layouts in this browser. Sociobot/Dodo is the merchant of record. Checkout, receipts, taxes, and refunds are handled there. A refund revokes the license automatically. Accessibility, safety information, and portfolio export remain free.</p>
...
function bindEvents(): void {
  ...
  document.querySelector<HTMLFormElement>("#license-form")?.addEventListener("submit", async (event) => {
    ...
    if (!token) return;
    storeLicense(token); unlocked = true; render(); toast("Checking the license…");
